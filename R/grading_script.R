...
ds4psych_grading <- function(path) {
  ...
  if (!simplegit::gh_collab_check(path, "smasongarrison", .token = my_pat, messages = FALSE)) {
    simplegit::gh_collab_invite(path, "smasongarrison", .token = my_pat)
    simplegit::gh_issue_new(path, title = "Please grade me!", body = "@smasongarrison, I've finished my lab. Please grade me!", .token = my_pat)
  } else {
    ...
    if (simplegit::gh_issue_mention(path, collaborator = "smasongarrison", .token = my_pat)) {
      # Assign Mason to those -- nothing happens if we reassign
      which_issues <- as.numeric(simplegit::gh_issue_list(path, mentioned = "smasongarrison", .token = my_pat)$number)
      lapply(which_issues, simplegit::gh_issue_assign, path = path, assignees = "smasongarrison", .token = my_pat)
